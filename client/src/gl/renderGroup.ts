...
function createRenderLayer(rmap: RenderMap, layer: Layer) {
  if (layer instanceof QuadsLayer) return new RenderQuadsLayer(rmap, layer)
  else if (layer instanceof GameLayer) return new RenderGameLayer(rmap, layer)
  else if (layer instanceof FrontLayer) return new RenderFrontLayer(rmap, layer)
  else if (layer instanceof TilesLayer) return new RenderTilesLayer(rmap, layer)
  else if (layer instanceof TeleLayer) return new RenderTeleLayer(rmap, layer)
  else if (layer instanceof SpeedupLayer) return new RenderSpeedupLayer(rmap, layer)
  else if (layer instanceof SwitchLayer) return new RenderSwitchLayer(rmap, layer)
  else if (layer instanceof TuneLayer) return new RenderTuneLayer(rmap, layer)
  else throw new Error('not a layer type we can render at the moment')
}
...
export class RenderGroup {
  ...
  private preRender() {
    ...
    if (this.group.clipping) this.setClipping()
  ...
  renderLayers(layers: RenderLayer[]) {
    ...
    for (const layer of layers) layer.render(viewBox)
