<script lang="ts">
  ...
  import { Automapper, parse as parseAutomapper, lint as lintAutomapper, LintLevel } from "../../twmap/automap";
  ...
  $: if (selectedIndex === -1) {
    layer.automapper.config = null
  } else {
    layer.automapper.config = configs[selectedIndex]
  }
  ...
  async function onFileChange(e: Event) {
    ...
    if (newRules) for (const rule of newRules) {
      configs[rule.name] = rule
    }

    showInfo(`Uploaded ${newRules.length} rules with ${errs.length} errors and ${warns.length} warnings.`, 'closable')
  }
